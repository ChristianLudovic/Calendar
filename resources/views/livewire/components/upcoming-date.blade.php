<div x-data="{ isExpanded: @entangle('isExpanded') }" 
    ...
    <div x-show="!isExpanded"
         ...
         class="flex justify-between items-center py-4 px-6 rounded-full bg-[#161618] w-full">
        <div class="flex space-x-4 items-center">
            <img src="{{ $imagePath }}" alt="{{ $name }}" class="w-12 h-12 rounded-full">
            <div class="">
                <h2 class="text-lg font-medium">{{ $name }}</h2>
                <p class="text-md text-[#6A696A]">{{ $date }}</p>
            </div>
        </div>
    ...
    <div x-show="isExpanded"
         ...
         x-transition:leave-start="opacity-100 scale-100"
         x-transition:leave-end="opacity-0 scale-90"
         class="bg-[#161618] rounded-[50px] w-full space-y-4 flex flex-col items-center justify-center h-full relative">
        <img src="{{ $imagePath }}" alt="{{ $name }}" class="w-18 h-18 rounded-full">
        <div class="text-center">
            <h2 class="text-2xl font-semibold tracking-[1px]">{{ $name }}</h2>
            <p class="text-md text-[#6A696A] font-medium">{{ $date }}</p>
        </div>
